...
mkdir -p "$MODEL_OUT/filtered"

# ---- 1) 生成训练标签（(ef_c,ef_w,L) 网格搜索）----
TRAIN_CSV="$MODEL_OUT/train_runs.csv"
"$ROOT/build/run_train_get_all_k_onlytop" "$DATA_DIR/clip-webvid-2.5M/" "$INDEX" \
...
"$ROOT/build/augment_features" "$DATA_DIR/clip-webvid-2.5M" "$FEAT_CSV" \
  --load_synth_prefix "$SYN_PREFIX" \
  --index "$INDEX" \
  --k "$K"
