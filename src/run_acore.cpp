// ./run_hnsw_cluster_consensus_real_test_light_newfeat_new2  ../data/clip-webvid-2.5M/ ../data/webvid_base.hnsw --clusters 10 --k 10 --ef 2000 --load_synth_prefix ./outputs_webvid_new_train2_test2/s1  --ef_warm 400 --model_dir model_out_txt_webvid_k10_without --model_A model_A_efc_mono.txt --model_B model_B_efw_mono.txt --model_rank model_rank_m_mono.txt --R_target 0.85:0.05:0.95 --csv_out webvid_k10_without_cluster_size.csv 

// ./run_hnsw_cluster_consensus_real_test_light_newfeat_new2_ablation \
//   ../data/clip-webvid-2.5M/ \
//   ../data/webvid_base.hnsw \
//   --clusters 60 \
//   --k 10 \
//   --ef 2000 \
//   --load_synth_prefix ./outputs_webvid_new_train_/s1 \
//   --model_dir ./exp_ablation_webvid2__train2_test2_2features__webvidfinal/webvid_final_k10 \
//   --model_A model_A_efc_mono.txt \
//   --model_B model_B_efw_mono.txt \
//   --model_rank model_rank_m_mono.txt \
//   --R_target 0.85:0.01:1.00 \	
//   --csv_out webvid_ind_new_train_corr_0.9.csv 
  

// run_hnsw_cluster_consensus.cpp 
// 需求对齐（新版）：
// 1) 取消三角不等式判断与“最远优先剔除”。
// 2) 对质心 c 做一次纯 HNSW L0 搜索（ef=args.ef），结束时导出 cand/top/visited 作为共享工具。
// 3) 簇内其他 query 直接用这套工具继续 L0 搜索（ef=ef_warm），各自产出结果。
// 4) L 的两种出法可选：--L_mode rankm（默认，需 model_rank_m.txt，可选 gate）；或 --L_mode recall（需 recall_head.txt）。
...
static Args parse_args(int argc, char** argv){
    ...
    for (; i<argc; ++i){
        ...
        else if (s=="--batch_size") a.batch_size=std::stoi(next());
...
int main(int argc, char** argv){
    ...
    std::vector<idx_t> active_qids;
    {
        ...
        if (args.batch_size > 0) {
            for (auto& C : clusters) {
                if ((int)C.size() > args.batch_size) C.resize((size_t)args.batch_size);
            }
        }
        ...
        if (args.batch_size > 0) {
            std::cout<<"[Oracle] batch_size_per_cluster="<<args.batch_size
                     <<" active_queries="<<active_qids.size()<<"/"<<nq<<"\n";
        }
    }
    idx_t eval_nq = active_qids.empty() ? nq : (idx_t)active_qids.size();
    ...
    // ---- 保存 CSV 行 ----
    struct Row { float target; double qps; float recall; double cache_mem_mb; };
    std::vector<Row> rows;
    ...
    for (size_t ridx = 0; ridx < target_list.size(); ++ridx){
        ...
        // ---- 锚点上下文缓存内存统计 ----
        // 每个簇缓存的 anchor context = topE (top IDs) + candE (candidate IDs)
        // 每条 entry = sizeof(tableint) = 4 字节
        size_t total_topE_bytes  = 0;   // 所有簇 topE 缓存总内存
        size_t total_candE_bytes = 0;   // 所有簇 candE 缓存总内存
        size_t total_vec_bytes   = 0;   // 所有簇注入节点向量数据总内存 (m_hat * dim * 4B)
        size_t max_ctx_bytes = 0;
        size_t min_ctx_bytes = std::numeric_limits<size_t>::max();
        size_t mem_clusters = 0;
        ...
        for (size_t ci = 0; ci < clusters.size(); ++ci){
            ...
            // 实际注入 = min(m_hat, topE.size())，topE 来自 efc_hat 搜索
            size_t actual_inj = std::min((size_t)std::max(0LL, (long long)std::llround(m_hat)), topE.size());

            // 锚点上下文缓存内存: topE + candE (均为 tableint ID 列表)
            size_t topE_bytes  = topE.size()  * sizeof(hnswlib::tableint);
            size_t candE_bytes = candE.size() * sizeof(hnswlib::tableint);
            size_t ctx_bytes   = topE_bytes + candE_bytes;
            // 注入节点向量数据内存 (若需单独缓存向量)
            size_t vec_bytes   = actual_inj * (size_t)dim * sizeof(float);

            total_topE_bytes  += topE_bytes;
            total_candE_bytes += candE_bytes;
            total_vec_bytes   += vec_bytes;
            if (ctx_bytes < min_ctx_bytes) min_ctx_bytes = ctx_bytes;
            if (ctx_bytes > max_ctx_bytes) max_ctx_bytes = ctx_bytes;
            ++mem_clusters;
            ...
            std::cout << std::fixed << std::setprecision(4)
                    ...
                    << "  m*=" << (double)m_hat
                    << "  Lpred=" << Lpred
                    << "  inj=" << actual_inj << "/" << topE.size()
                    << "  ctx_cache=" << std::setprecision(2)
                      << ((double)ctx_bytes / 1024.0) << " KB"
                    << "\n";
        ...
        // ---- 锚点上下文缓存内存统计摘要 ----
        if (mem_clusters > 0) {
            size_t total_ctx_bytes = total_topE_bytes + total_candE_bytes;
            double total_ctx_mb = (double)total_ctx_bytes / (1024.0 * 1024.0);
            double avg_ctx_kb   = (double)total_ctx_bytes / mem_clusters / 1024.0;
            double total_vec_mb = (double)total_vec_bytes  / (1024.0 * 1024.0);
            std::cout << "\n[Anchor Context Cache Cost] clusters=" << mem_clusters
                      << "  total_cached=" << std::fixed << std::setprecision(2)
                      << total_ctx_mb << " MB"
                      << "  avg_per_cluster=" << avg_ctx_kb << " KB"
                      << "  min=" << ((double)min_ctx_bytes / 1024.0) << " KB"
                      << "  max=" << ((double)max_ctx_bytes / 1024.0) << " KB"
                      << "\n"
                      << "  [breakdown] topE=" << ((double)total_topE_bytes / 1024.0) << " KB"
                      << "  candE=" << ((double)total_candE_bytes / 1024.0) << " KB"
                      << "  injected_vecs=" << total_vec_mb << " MB"
                      << "  (tableint=" << sizeof(hnswlib::tableint) << "B, dim=" << dim << ")"
                      << "\n";
        }
        ...
        double qps = eval_nq / total_time_seconds;

        // 计算 recall
        float recall = compute_recall_subset(out_topk, gt, args.k, active_qids.empty() ? std::vector<idx_t>{} : active_qids);
        if (active_qids.empty()) {
            std::vector<idx_t> all_qids(nq);
            std::iota(all_qids.begin(), all_qids.end(), (idx_t)0);
            recall = compute_recall_subset(out_topk, gt, args.k, all_qids);
        }
        ...
        // 计算每个 query 的 recall（使用已裁剪的 gt: nq x k）
        {
            ...
            const std::vector<idx_t>* qids = &active_qids;
            std::vector<idx_t> all_qids;
            if (qids->empty()) {
                all_qids.resize(nq);
                std::iota(all_qids.begin(), all_qids.end(), (idx_t)0);
                qids = &all_qids;
            }
            for (idx_t orig_qi : *qids){
                int hit = 0;
                const auto& R = out_topk[orig_qi];
                for (int j=0; j<expect; ++j){
                    int g = gt[(size_t)orig_qi * per_row + j];
                    if (g < 0) break;
                    for (int t=0; t<std::min((int)R.size(), args.k); ++t){
                        if ((int)R[t] == g){ ++hit; break; }
                    }
                }
                per_query_recall[orig_qi] = (expect>0)? ((float)hit / (float)expect) : 0.0f;
            }
        }
        ...
        std::cout << "TargetRecall="<< std::fixed << std::setprecision(4) << args.R_target
                  << " | Base: nb="<<nb<<" dim="<<dim<<"  Queries: nq="<<eval_nq;
        if (eval_nq != nq) std::cout << " (total="<<nq<<")";
        std::cout << "\n";
        ...
        rows.push_back(Row{args.R_target, qps, recall,
                           (mem_clusters > 0)
                             ? ((double)(total_topE_bytes + total_candE_bytes) / (1024.0 * 1024.0))
                             : 0.0});
        ...
        if (!args.per_query_csv_out.empty()){
            ...
            if (!qout){
                ...
                std::cout << "[Per-Query CSV] wrote nq=" << qids->size() << " rows to " << args.per_query_csv_out << "\n";
            }
        }
    }
    ...
    // ---- 写出 CSV ----
    {
        ...
        if (!fout){
            ...
            fout << "target_recall,qps,recall,cache_mem_mb\n";
            fout << std::fixed << std::setprecision(6);
            for (const auto& r : rows){
                fout << r.target << "," << r.qps << "," << r.recall << "," << r.cache_mem_mb << "\n";
            }
