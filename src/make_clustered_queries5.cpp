// make_clustered_queries2.cpp
// 生成“多簇、每簇大小与角度可不同”的查询集，并可选计算 GT 与 Jaccard 统计。
// 编译：d
// ./make_clustered_queries5 \
//   --real_q ../data/clip-webvid-2.5M/query.10k.fbin \
//   --out_prefix outputs_webvid_new_train_test_0111/s1  \
//   --clusters 10 \
//   --qpc_min 80 --qpc_max 200 --qpc_dist uniform --qpc_logmean 4.6 --qpc_logstd 0.4 \
//   --sigma_deg_min 2 --sigma_deg_max 6 \
//   --theta_min_deg_min 1.0 --theta_min_deg_max 2.0 \
//   --min_sep_deg 60 \
//   --noise_model tangent_gauss \
//   --per_query_sigma_logstd 0.12 \
//   --anisotropy_lambda 1.15 \
//   --center_jitter_deg 0.5 \
//   --heavy_tail_p 0 \
//   --base ../data/clip-webvid-2.5M/base.2.5M.fbin --k_gt 100 --normalize_base 1 \
//   --seed 66 --nthreads 24
//   ./make_clustered_query5 \
//     --real_q ../data/laion-10M/query.10k.fbin \
//     --out_prefix outputs_laion_new_train_clusters/s1 \
//     --clusters 30 \
...
//     --min_sep_deg 60 \
//     --base ../data/laion-10M/base.10M.fbin --k_gt 100 --normalize_base 1 \
//     --seed 42 --nthreads 24
// 典型用法 A（每簇参数来自配置文件）:
//   ./make_clustered_queries2 \
//     --real_q ../data/clip-webvid-2.5M/query.10k.fbin \
//     --out_prefix outputs_webvid_new_train/s1 \
//     --clusters 40 --clusters_cfg clusters_cfg.txt \
//     --min_sep_deg 60 \
//     --base ../data/clip-webvid-2.5M/base.2.5M.fbin --k_gt 100 --normalize_base 1 \
//     --seed 66 --nthreads 24
//   # clusters_cfg.txt 每行: qpc,sigma_deg,theta_min_deg
//
// 典型用法 B（纯命令行分布采样，不写配置文件）:
//   ./make_clustered_query5 \
//     --real_q ../data/laion-10M/query.10k.fbin \
//     --out_prefix outputs_laion_new_train2_test2/s1 \
//     --clusters 50 \
//     --qpc_min 60 --qpc_max 300 --qpc_dist uniform --qpc_logmean 5.3 --qpc_logstd 0.6 \
//     --sigma_deg_min 5 --sigma_deg_max 35 \
//     --theta_min_deg_min 2 --theta_min_deg_max 12 \
//     --min_sep_deg 60 \
//     --base ../data/laion-10M/base.10M.fbin --k_gt 100 --normalize_base 0 \
//     --seed 42 --nthreads 24
//
// 只算 GT（已存在的 out_prefix.xq.fbin 上重算 GT）：
//   ./make_clustered_queries2 --gt_only \
//     --real_q ../data/clip-webvid-2.5M/query.10k.fbin \
//     --out_prefix outputs_webvid_new_train/s1 \
//     --base ../data/clip-webvid-2.5M/base.2.5M.fbin --k_gt 100 --normalize_base 1 \
//     --nthreads 24
// 新增 --noise_model=tangent_gauss：在中心向量的切平面采样高斯噪声 g ~ N(0, σ_t^2 I)，令 q = normalize(c + g)。

// 语义保持：仍然用现有的 --sigma_deg 表示角度标准差（度）。在切平面高斯里，小角度下有近似 θ ≈ ||g||，而 ||g||^2 ~ σ_t^2 χ^2_{d-1}，因此取
// σ_t = (sigma_rad) / sqrt(d-1)（sigma_rad = sigma_deg * π/180）即可让角度的标准差大致等于 sigma_deg。

// 可选增强：

// --per_query_sigma_logstd：每个 query 的尺度作 lognormal 抖动（异质性）。

// --heavy_tail_p + --heavy_tail_mult：以小概率放大尺度，制造重尾（偶尔离中心远很多）。

// --anisotropy_lambda：在切平面上引入 rank-1 各向异性（沿某一随机切向方向方差 × λ）。

// --center_jitter_deg：对每个簇的中心做轻微抖动，模拟质心估计误差。
// ./make_clustered_query4 \
//   --real_q ../data/clip-webvid-2.5M/query.10k.fbin \
//   --out_prefix outputs_webvid_new_train_gauss/s1_gauss \
//   --clusters 10 \
//   --qpc_min 60 --qpc_max 300 --qpc_dist lognormal --qpc_logmean 5.3 --qpc_logstd 0.6 \
//   --sigma_deg_min 5 --sigma_deg_max 35 \
//   --theta_min_deg_min 2 --theta_min_deg_max 12 \
//   --min_sep_deg 60 \
//   --noise_model tangent_gauss \
//   --per_query_sigma_logstd 0.3 \
//   --heavy_tail_p 0.05 --heavy_tail_mult 3.0 \
//   --anisotropy_lambda 2.0 \
//   --center_jitter_deg 1.5 \
//   --base ../data/clip-webvid-2.5M/base.2.5M.fbin --k_gt 100 --normalize_base 1 \
//   --seed 66 --nthreads 24
//   ./make_clustered_query5 \
//     --real_q ../data/clip-webvid-2.5M/query.10k.fbin \
//     --out_prefix outputs_webvid_new_train_clusters/s1 \
//     --clusters 30 \
//     --qpc_min 60 --qpc_max 300 --qpc_dist uniform --qpc_logmean 5.3 --qpc_logstd 0.6 \
//     --sigma_deg_min 35 --sigma_deg_max 75 \
//     --theta_min_deg_min 2 --theta_min_deg_max 12 \
