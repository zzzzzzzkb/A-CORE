...
# -*- coding: utf-8 -*-
# 用法示例：
# python src/train_full_conditional_and_recall_newfeat_ablation.py \
#   --csv ./filtered_csvs/filtered_AB_rows.with_new_feats.csv \
#   --outdir ./model_out_txt_newfeat_ablation \
#   --alpha 0.7 --seed 42
"""
与 train_full_conditional_and_recall_newfeat.py 基本一致，唯一区别：
- 新增 ABLATION_KEEP消融“保留”列表初始化包含所有可用特征
- 训练前对每个 head 的特征做过滤：仅使用 ABLATION_KEEP 中仍保留的特征；
- 你只需在 ABLATION_KEEP 中删掉不想使用的特征即可完成消融。
...
def lgb_reg_fit(X, y, seed=42, mono=None, use_huber=True):
    params = dict(
        ...
        num_leaves=63, subsample=0.9, colsample_bytree=0.9,
        min_data_in_leaf=40, min_gain_to_split=5e-3,
        reg_lambda=3.5, max_bin=255, random_state=seed,
    )
    if mono is not None:
