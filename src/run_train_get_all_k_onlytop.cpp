// run_hnsw_collect_training.cpp (OpenMP-accelerated, multi-k recalls + k-wise overlap/jaccard + LID/RC/Expansion@k=10)
//
// 输出字段（严格顺序）：
...
//
// 主要改动：
// - 新增 --k_collect（默认 100），一次检索保留前 k_collect 个结果，统一派生 recall@1/10/100；
// - overlap/jaccard 同时计算 K=1/10/100 六列（并保留原“默认K=10”的两列以兼容）；
// - 新增三列：LID/RC/Expansion（在 smallEF=256 的质心 L0 top 上，k=10 计算）；
// - 其它流程与并行保持不变。
...
int main(int argc, char** argv){
    ...
    // 主循环：参数网格 × 簇 —— 按簇并行
    // 优化：质心搜索（ef=efc）只依赖 (efc, cluster)，缓存后供所有 (efw, L) 复用
    struct CentroidCache {
        std::vector<hnswlib::tableint> top_ids;
        int centroid_steps = 0;
        double centroid_time = 0.0;
    };

    for(int efc: args.efc_list){
        // ---- 缓存阶段：对每个簇做一次质心搜索（ef=efc） ----
        std::vector<CentroidCache> cc(clusters.size());
        {
            ...
            for(long long ci_ll=0; ci_ll<(long long)clusters.size(); ++ci_ll){
                ...
                cc[ci].top_ids = std::move(top_ids);
                cc[ci].centroid_steps = centroid_steps;
                cc[ci].centroid_time = std::chrono::duration<double>(t1-t0).count();
            }
            auto cache_end = std::chrono::high_resolution_clock::now();
            std::cout<<"[cache] efc="<<efc<<" cached "<<clusters.size()<<" clusters in "
                     <<std::chrono::duration<double>(cache_end-cache_start).count()<<"s\n";
        }
        ...
        for(int efw: args.efw_list){
            ...
            for(float L_in: args.L_list){
                ...
                for(long long ci_ll=0; ci_ll<(long long)clusters.size(); ++ci_ll){
                    ...
                    // 1) 使用缓存的质心搜索结果（ef=efc）
                    const auto& top_ids = cc[ci].top_ids;
                    int centroid_steps = cc[ci].centroid_steps;
                    double centroid_time = cc[ci].centroid_time;
                    std::vector<hnswlib::tableint> dummy_cand_ids;  // continueFromSnapshotL0 中未使用
